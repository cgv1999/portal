...
function formatCurrency(value) {
    ...
    if (absValue >= 1000000) {
        ...
        return inMillions.toFixed(1) + ' млн ₽';
    } else if (absValue >= 1000) {
        return (roundedValue / 1000).toFixed(0) + ' тыс ₽';
    }
...
function populateFilters(data) {
    ...
    // Убираем "Все объекты", оставляем только "Все франчайзи"
    objectSelect.innerHTML = '<option value="">Выберите объект</option><option value="all_franchise">Все франчайзи</option>';
...
function updateSelectedInfo(containerId, selectElement) {
    ...
    if (selectedOptions.length > 0) {
        ...
        } else if (selectElement.id === 'objectSelect') {
            container.textContent = 'Выберите объект';
        } else {
...
// Построение графика с измененным порядком столбцов
function buildChart(data, selectedObjects) {
    ...
    // Изменяем порядок серий: сначала прибыль, потом выручка
    const option = {
        title: {
            text: titleText,
            subtext: subtitle,
        ...
        legend: {
            data: ['Операционная прибыль', 'Выручка'], // Меняем порядок в легенде
            left: '10%',
            top: 50,
        },
        tooltip: {
            ...
            formatter: function (params) {
                let result = `<div style="text-align: left; padding: 5px;">`;
                ...
                const sortedParams = [...params].sort((a, b) => {
                    const order = { 'Операционная прибыль': 1, 'Выручка': 2 };
                    return (order[a.seriesName] || 99) - (order[b.seriesName] || 99);
                });
                ...
                sortedParams.forEach(param => {
                    ...
                    result += `<div style="display: flex; align-items: center; margin: 3px 0;">
                        <span style="display: inline-block; width: 10px; height: 10px; background: ${color}; border-radius: 2px; margin-right: 5px;"></span>
                        ${seriesName}: <strong style="margin-left: auto;">${value}</strong>
                    </div>`;
                });
                ...
                result += `<div style="color: ${profitColor}; font-weight: bold; margin-top: 3px;">
        ...
        series: [
            // Первый столбец - Операционная прибыль (слева)
            {
